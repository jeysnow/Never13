    ...
    public class AndroidClient : IPlayGamesClient
    {
        ...
        public void Authenticate(bool silent, Action<SignInStatus> callback)
        {
            lock (AuthStateLock)
            {
                ...
                {
                    Debug.Log("myTest:"+"Already authenticated.");
            ...
            Debug.Log("myTest:"+"Starting Auth with token client.");
            mTokenClient.FetchTokens(silent, (int result) =>
            {
                ...
                else
                {
                    lock (AuthStateLock)
                    {
                        Debug.Log("myTest:"+"Returning an error code.");
        ...
        public string GetUserEmail()
        {
            ...
            {
                Debug.Log("myTest:"+"Cannot get API client - not authenticated");
                return null;
            }
        ...
        public void GetPlayerStats(Action<CommonStatusCodes, PlayerStats> callback)
        {
            ...
            using (var task = playerStatsClient.Call<AndroidJavaObject>("loadPlayerStats", /* forceReload= */ false))
            {
                ...
                AddOnFailureListenerWithSignOut(
                    ...
                    e =>
                    {
                        Debug.Log("myTest:"+"GetPlayerStats failed: " + e.Call<string>("toString"));
                ...
                for (int i = 0; i < count; ++i)
                {
                    using (var task = playersClient.Call<AndroidJavaObject>("loadPlayer", userIds[i]))
                    {
                        ...
                        AddOnFailureListenerWithSignOut(
                            ...
                            exception =>
                            {
                                Debug.Log("myTest:"+"LoadUsers failed for index " + i + " with: " + exception.Call<string>("toString"));
                                lock (countLock)
                                {
                                    ++resultCount;
                                    if (resultCount == count)
        ...
        public void LoadAchievements(Action<Achievement[]> callback)
        {
            ...
            using (var task = achievementsClient.Call<AndroidJavaObject>("load", /* forceReload= */ false))
            {
                ...
                AddOnFailureListenerWithSignOut(
                    ...
                    exception =>
                    {
                        Debug.Log("myTest:"+"LoadAchievements failed: " + exception.Call<string>("toString"));
            ...
            using (var client = getLeaderboardsClient())
            {
                    ...
                    rowCount))
                {
                    ...
                    AddOnFailureListenerWithSignOut(
                        ...
                        exception =>
                        {
                            Debug.Log("myTest:"+"LoadScores failed: " + exception.Call<string>("toString"));
            ...
            Action<LeaderboardScoreData> callback)
        {
                ...
                token.InternalObject, rowCount, AndroidJavaConverter.ToPageDirection(token.Direction)))
            {
                ...
                AddOnFailureListenerWithSignOut(
                    ...
                    exception =>
                    {
                        Debug.Log("myTest:"+"LoadMoreScores failed: " + exception.Call<string>("toString"));
                        InvokeCallbackOnGameThread(callback,
